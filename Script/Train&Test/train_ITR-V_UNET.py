...
def fit(epochs, model, train_loader, val_loader, criterion, optimizer, scheduler, writer, device, n_classes, patch=False, args=None):
    ...
    for e in range(epochs):
        ...
        color = ['black', 'yellow']
        # color = ['black', 'yellow', 'red', 'green', 'purple']
...
def main():
    ...
    val_img_folder = os.path.join(root, dataset, "/mnt/Dataset/Single_Patch/ITR_V/Validation/Images")
    val_mask_folder = os.path.join(root, dataset, "/mnt/Dataset/Single_Patch/ITR_V/Validation/Masks")
    ...
    t_train = A.Compose([A.Resize(320, 512, interpolation=cv2.INTER_LINEAR), A.CenterCrop(320,320), A.Resize(608, 608, interpolation=cv2.INTER_LINEAR), A.HorizontalFlip(p=0.5), A.VerticalFlip(p=0.5),
                        A.GridDistortion(p=0.25), A.RandomRotate90(p=0.5), A.Transpose(p=0.5), A.RandomBrightnessContrast((0,0.5),(0,0.5),p=0.2),
                        A.GaussNoise(p=0.5)])

    t_val = A.Compose([A.Resize(320, 512, interpolation=cv2.INTER_LINEAR), A.CenterCrop(320,320), A.Resize(608, 608, interpolation=cv2.INTER_LINEAR), A.HorizontalFlip(p=0.5), A.VerticalFlip(p=0.5),
