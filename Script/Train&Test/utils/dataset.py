...
class SemSegDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #   img = Image.fromarray(img)
            mean = [0.485, 0.456, 0.406]
            std = [0.229, 0.224, 0.225]
            t = T.Compose([T.Resize((1430, 1920)), T.CenterCrop(1430), T.Resize((512, 512)), T.ToTensor(), T.Normalize(mean, std)])
            #t = T.Compose([T.ToTensor(), T.Normalize(mean, std)])
            img = t(img)
            t = T.Compose([T.Resize((1430, 1920)), T.CenterCrop(1430), T.Resize((512, 512)), T.ToTensor()])
            #t = T.Compose([T.ToTensor()])
            mask = t(mask).squeeze()
            mask = (mask>0).long()
